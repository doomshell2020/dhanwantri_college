 ...
 <th style="width: 15%;">Name</th>
  ...
  <th style="width: 14%;">Mother Name</th>
	  ...
	  <? if($work['category']!="RTE"){ ?>
		    ...
		    <a class='global1' title="Pending Fees" style="color:red;" href="<?php echo SITE_URL; ?>admin/students/view_out/<?php echo $work['id']; ?>" 
                 data-target="#global-drop-outs<?php echo $work['id']; ?>" data-toggle="modal"><img src="<? echo SITE_URL; ?>images/pending.png" ></a>
                ...
                <a  title="Tution Fees Acknowledgement  <?php echo $work['acedmicyear']; ?>"  target="_blank" href="<?php echo SITE_URL; ?>admin/report/feeacknowledgement/<?php echo 
                  ...
                  <div class="modal" id="global-drop-outs<?php echo $work['id']; ?>" tabindex="-1" role="dialog" aria-labelledby="esModalLabel" aria-hidden="true" 
                  style="display: none;">
    ...
    $('.modal-content-drop-out').html('');
      ...
      <td><input type="text" value="<?php echo $work['fname']; ?> <?php  echo $work['middlename']; ?> <?php echo $work['lname']; ?>" 
       class="form-control" name="name[]" placeholder="Enter Pupil's Name"><?php  if($work['discountcategory']){ echo 
					   "<span style='color:green;font-weight:12px;'><b>".$work['discountcategory']."</b></span>";  } ?>
			    </td>
       <td>   <input type="text" value="<?php echo $work['fathername'] ?>"class="form-control" 
			    name="fathername[]" ></td>
			        <td>   <input type="text" value="<?php echo $work['mothername'] ?>"class="form-control" 
			    name="mothername[]" ></td>
...
-->
 <input type="hidden" value="<?php echo $work['id']; ?>" class="form-control" 
			    name="id[]">
<?php  if($work['category']=="RTE"){ echo 
	$this->Form->input('class_id[]',array('class'=>'form-control','type'=>'select','disabled'=>'disabled','id'=>'class-idss'.$work['class_id'],'value'=>$work['class_id'],'options'=>$classes,'label' =>false,)); ?>
	 ...
	 <input type="hidden" value="<?php echo $work['class_id']; ?>" class="form-control" 
			    name="class_id[]">
	<?php }else{
...
echo $this->Form->input('section_id[]',array('class'=>'form-control','type'=>'select','id'=>'section-idss'.$work['section_id'],'value'=>$work['section_id'],'options'=>$sectionslist,'label' =>false)); ?> </td>
           <td>
	 <input type="text" value="<?php echo $work['sms_mobile']; ?>" class="form-control" 
			    name="sms_mobile[]" placeholder="Enter  First Name">
			  </td>
