...
if (file_exists($filename2)) {  ?>	
    ...
    margin-top: -17px;"class="center-block img-circle img-thumbnail profile-img" src="<?php echo SITE_URL;?>stu/<?php echo 
                ...
                <a href="#"><?php echo ucfirst($students['fname']); ?> <?php echo ucfirst($students['middlename']); ?> <?php echo ucfirst($students['lname']); ?></a>   (<b style="color:green;"><?php echo $students['enroll']; ?></b>)        
                    ...
                    <? if($students['due_fees']){  $findpendings=$this->Comman->studentshistory($students['stud_id']);?>    <strong class="pull-right" style="color:red;">Pending history : <span class="text-black">₹ </span><? echo $students['due_fees']; ?> <a href="<? echo ADMIN_URL; ?>studentfees/history/<? echo $findpendings['stud_id']; ?>/<? echo $findpendings['acedmicyear']; ?>" class="btn btn-info " > <? echo $findpendings['acedmicyear']; ?> </a> </strong> <? }else if($acedmicyears){   $dff=$this->Comman->findridacademicreerty($students['stud_id'],$acedmicyears); 
...
<script type="text/javascript">
 ...
 if($('.StuAttendCkcancel:checked').length==0) {
	 $('#chkotherfee').prop('required',false);
			...
			<?php $dats= date('Y-m-d',strtotime($valsf['paydate'])); if($dats!='1970-01-01'){ echo date('d-m-Y',strtotime($dats)); $acedmicdd=date('Y',strtotime($dats));  $previ=explode('-',$academic_year); }else{ echo "not-set"; } ?>
