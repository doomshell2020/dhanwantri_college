...
class ReportController extends AppController
{
    ...
    private function arrayToCollection($array)
    {
        $collection = new \Cake\Collection\Collection($array);
        return $collection;
    }
    ...
    public function detailreport()
    {
        ...
        $classes = $this->Classections->find('list', ['keyField' => 'Classes.id', 'valueField' => 'Classes.title'])->contain(['Classes'])->where(['Classections.status' => 'Y'])->order(['Classes.sort' => 'ASC'])->toArray();
        $ar = array('2', '3');
        $feesheadstotal = $this->Feesheads->find('all', ['keyField' => 'id', 'valueField' => 'name'])->where(['status' => 'Y', 'sort !=' => '0'])->order(['sort' => 'ASC'])->toArray();
        $users = $this->Users->find('all')->where(['Users.id' => '1'])->first();
        $acedmic = $users['academic_year'];
        $this->set(compact('acedmic', 'feesheadstotal', 'classes'));
    }
    ...
    public function document()
    {
        ...
        $documentcategory = $this->Documentcategory->find('list', ['keyField' => 'id', 'valueField' => 'categoryname'])->order(['id' => 'DESC'])->toArray();

        $this->loadModel('Students');
        $this->loadModel('DropOutStudent');
        $sms_temp_id = $_GET['document_id'];
        $enroll = $_GET['enroll'];
        $apk = array();
        ...
        if (!empty($apk)) {
            $students = $this->Documents->find('all')->contain(['Students', 'Documentcategory'])->where($apk)->order(['Documents.id' => 'DESC', 'Students.fname' => 'ASC']);
        } else {
            $students = $this->Documents->find('all')->contain(['Students', 'Documentcategory'])->order(['Documents.created' => 'DESC']);
        }
        $students = $this->paginate($students)->toArray();
        $this->set(compact('students', 'documentcategory'));
    }
    ...
    public function documentdropout()
    {
        ...
        $this->request->session()->delete('students');
        $this->viewBuilder()->layout('admin');
        $documentcategory = $this->Documentcategory->find('list', ['keyField' => 'id', 'valueField' => 'categoryname'])->where(['Documentcategory.type !=' => 0])->order(['id' => 'DESC'])->toArray();
    ...
    public function age()
    {
        ...
        if ($rolepresent == ADMIN) {
            $classes = $this->Classections->find('list', ['keyField' => 'Classes.id', 'valueField' => 'Classes.title'])->contain(['Classes'])->where(['Classes.status' => '1'])->order(['Classes.sort' => 'ASC'])->toArray();
        } elseif ($rolepresent == CBSE_FEE_COORDINATOR) {
            $classes = $this->Classections->find('list', ['keyField' => 'Classes.id', 'valueField' => 'Classes.title'])->contain(['Classes'])->where(['Classes.status' => '1', 'Classes.board_id IN' => ALL_BOARDS])->order(['Classes.sort' => 'ASC'])->toArray();
    ...
    public function searchdailyreport()
    {
        ...
        $classes = $this->Classections->find('list', [
            'keyField' => 'Classes.id',
            'valueField' => 'Classes.title',
        ])->contain(['Classes'])->where(['Classes.status' => '1', 'Classes.board_id IN' => ALL_BOARDS])->order(['Classes.title' => 'ASC'])->toArray();
        // pr($classes); die;
        $this->set('classes', $classes);
    ...
    public function searchdailyreport2()
    {
        $requestData = $this->request->data;
        // pr($requestData);exit;
        $mode = $requestData['mode'];
        $selected_month = date('Y-m', strtotime($requestData['selected_month']));
        ...
        // Get fee fields
        $selectField = $requestData['selectField'] ?? [];
        ...
        $fieldKeys = array_flip($selectField);
        $extraFields = array_flip(['Late Fee', 'Discount Fee', 'Other Discount']);
        ...
        $conditions = [
            ...
            'Studentfees.paydate >=' => $startDate,
            'Studentfees.paydate <=' => $endDate
        ];
        if (!empty($mode)) {
            $conditions['Studentfees.mode IN'] = $mode;
        }

        // Step 1: Fetch raw fee and dropout fee data
        $fees = $this->Studentfees->find('all')
            ->contain(['Students'])
            ->where($conditions)
            ->select(['Studentfees.id'])
            ->order(['Studentfees.paydate' => 'ASC'])
            ->toArray();

        $dropoutFees = $this->Studentfees->find('all')
        ...
        // Step 2: Extract IDs and make them unique
        $allFeeIds = array_merge(
            array_map(fn($f) => $f->id, $fees),
            array_map(fn($d) => $d->id, $dropoutFees)
        );
        $uniqueFeeIds = array_unique($allFeeIds);

        // Step 3: Now fetch final Studentfees records based on unique IDs
        $allFees = $this->Studentfees->find('all')
            ->contain(['Students', 'DropOutStudent']) // include both relations if needed
            ->where(['Studentfees.id IN' => $uniqueFeeIds])
            ->order(['Studentfees.paydate' => 'ASC'])
            ->toArray();
        ...
        $this->set([
            ...
            'acedmicyear' => $this->Users->get(1)->academic_year
        ]);
    }
    ...
    public function exportmonthlyexcel()
    {
        ...
        // Format start and end dates of the selected month
        $start_date = date('Y-m-01', strtotime($month));
        $end_date = date('Y-m-t', strtotime($month));

        // Get session data
        $allFees = $session->read('allFees') ?? [];
        $mode = $session->read('mode') ?? [];
        // pr($start_date);exit;
        // Set values for use in the view (Excel generation happens in the view)
        $this->set(compact('allFees', 'start_date', 'end_date', 'mode', 'month'));
    }
    ...
    public function searchcollectionfee()
    {
        ...
        if ($acedmicyear != "") {
            ...
            $apk[] = $pii2;
            if (!empty($class_id)) {
                $pii = array('Students.class_id IN' => $class_id);
                $apk[] = $pii;
            }
            if (!empty($mode)) {
                $pii = array('Studentfees.mode IN' => $mode);
            ...
            $apk2[] = $pii2;
            if (!empty($class_id)) {
                $pii = array('Studentshistory.class_id IN' => $class_id);
                $apk2[] = $pii;
            }
            ...
            $apk3[] = $pii2;
            if (!empty($class_id)) {
                $pii = array('DropOutStudent.laststudclass IN' => $class_id);
                $apk3[] = $pii;
            }
    ...
    public function students_allmonthly()
    {
        ...
        $classfee = $this->Classfee->find('all')->contain(['Classes'])->group('Classfee.academic_year')->group('Classfee.class_id')->where(['Classfee.academic_year' => $academicyear])->order(['Classfee.id' => 'ASC'])->select([
            'qu1_fees' => $this->Classfee->find()->func()->sum('Classfee.qu1_fees'),
        ...
        $rolepresent = $this->request->session()->read('Auth.User.role_id');
        // pr($rolepresent); die;
        if ($rolepresent == '1' || $rolepresent == '6' || $rolepresent == '14') {
            $classes = $this->Classections->find('list', [
                'keyField' => 'Classes.id',
                'valueField' => 'Classes.title',
            ])->contain(['Classes'])->where(['Classes.status' => '1'])->order(['Classes.sort' => 'ASC'])->toArray();
    ...
    public function students_all()
    {
        ...
        if ($rolepresent == '1' || $rolepresent == '6' || $rolepresent == '105' || $rolepresent == '14') {
            $classes = $this->Classes->find('list', [
                'keyField' => 'id',
                'valueField' => 'title',
            ])->where(['Classes.status' => '1'])->order(['Classes.sort' => 'ASC'])->toArray();
    ...
    public function user_dailyfee()
    {
        ...
        $classes = $this->Classections->find('list', ['keyField' => 'Classes.id', 'valueField' => 'Classes.title'])->contain(['Classes'])
            ->where(['Classections.status' => 'Y'])->order(['Classes.title' => 'ASC'])->toArray();
        //pr($classes); die;
        $this->set(compact('classes'));
    }
    ...
    public function user_collectionfee()
    {
        ...
        $headerRow2[] = "Refrence No.";
        // $headerRow2[] = "Refrence ddNo.";
        // $headerRow2[] = "Refrence bank.";
            ...
            if (!in_array($element['id'], $idds)) {
                ...
                if ($element['type'] == "Fee") {
                    ...
                    if ($stiiu['id']) {
                        ...
                        $result[] = $stiiu34['enroll'];
                        $result[] = $clsasss['title'];
                        $result[] = $section['title'];
                        $result[] = $stiiu34['batch'];
                    } else {
                ...
                if (!empty($element['cheque_no'])) {
                    if ($element['bank']) {
                        $result[] = "Recived cheque (" . $element['cheque_no'] . ") from " . $element['bank'];
                    } else {
                        $result[] = $element['cheque_no'];
                    }
                } elseif ($element['ref_no']) {
                    if ($element['bank']) {
                ...
                // if (in_array("NETBANKING", $mode)) {
                //     if ($element['ref_no']) {
                //         $result[] = $element['ref_no']."NETBANKING";
                //     } else {

                //         $result[] = "--"."NETBANKING";
                //     }
                //     if ($element['bank']) {
                //         $result[] = $element['bank']."NETBANKING";
                //     } else {
                ...
                // } elseif (in_array("Credit Card/Debit Card/UPI", $mode)) {
                //     if ($element['ref_no']) {
                //         $result[] = $element['ref_no']."Card";
                //     } else {

                //         $result[] = "--"."NETBANKING";
                //     }
                //     if ($element['bank']) {
                //         $result[] = $element['bank']."Card";
                //     } else {
        ...
        if (in_array("CHEQUE", $mode)) {
            ...
            $iio++;
            $iio++;
        } elseif (in_array("NETBANKING", $mode)) {
            $iio++;
            $iio++;
        } elseif (in_array("Credit Card/Debit Card/UPI", $mode)) {
            $iio++;
    ...
    // New Fees Report by rajesh kumar 26-07-24
    function feesreport()
    {
        $session = $this->request->session();
        $searchCond = $session->read('parentlogindata');
        $students = $this->Students->find('all')->contain(['Classes', 'Sections'])->where(['Students.status' => 'Y', $searchCond])->order(['Classes.sort' => 'ASC', 'Students.section_id' => 'ASC', 'Students.fname' => 'ASC'])->toarray();
        if (empty($students)) {
    ...
    public function findamount($id, $a_year)
    {
        ...
        return $this->Classfee->find('all')->contain(['Classes'])->group('Classfee.academic_year')->group('Classfee.class_id')->select([
            'qu1_fees' => $this->Classfee->find()->func()->sum('Classfee.qu1_fees'),
            'qu2_fees' => $this->Classfee->find('all')->func()->sum('Classfee.qu2_fees'),
            'qu3_fees' => $this->Classfee->find('all')->func()->sum('Classfee.qu3_fees'),
            'qu4_fees' => $this->Classfee->find('all')->func()->sum('Classfee.qu4_fees'),
            'Classes.title',
            'Classfee.academic_year',
            'Classfee.id',
            'Classfee.status',
    ...
    public function dailyreportprintinhtml()
    {
        $session = $this->request->session();
        $datefrom = $session->read('datefrom');
        $this->set(compact('datefrom'));
        ...
        if (!empty($dateto) && $dateto != '1970-01-01') {
            $this->set(compact('dateto'));
            $this->set(compact('dateto2'));
            $stts = array('Studentfees.paydate <=' => $dateto);
            $apk[] = $stts;
        }
        if (!empty($mode)) {
            $pii = array('Studentfees.mode IN' => $mode);
    ...
    //this function for Fees Collection Session Report

    // public function exportcollectioninhtml()
    // {
    //     $this->viewBuilder()->Layout('newadmin');
    // }
    ...
    // Hostel Fees Report (By Rupam) excel export code
    function hostelfeesreport()
    {
        $this->loadModel('Feesheads');

        $session = $this->request->session();
        $searchCond = $session->read('hostelfeesearch');
        $session->delete('hostelfeesearch');

        if (!empty($searchCond)) {
            $students = $this->Students->find('all')->contain(['Classes', 'Sections'])->where(['Students.is_hostel IS NOT' => null, 'Students.status' => 'Y', $searchCond])->order(['Classes.sort' => 'ASC'])->toarray();
        } else {
            $students = $this->Students->find('all')->contain(['Classes', 'Sections'])->where(['Students.is_hostel IS NOT' => null, 'Students.status' => 'Y'])->order(['Classes.sort' => 'ASC'])->toarray();
        }
        ...
        foreach ($students as $value) {
            ...
            if ($value['batch'] > '2022-23') {
                ...
                $asignHostel = $this->HostelFeesManagement->find('all')->where(['student_id' => $value['id']])->order(['HostelFeesManagement.id' => 'ASC'])->toarray();
                foreach ($asignHostel as $hostelDetails) {
                    $checkinDate = date('d-m-Y', strtotime($hostelDetails['checkin_date']));
                    $hostelScheme[] = $hostelDetails['fee_head_name'];

                    if (date('Y', strtotime($hostelDetails['checkout_date'])) != '1970') {
                        $currentDate = date('d-m-Y', strtotime($hostelDetails['checkout_date']));
                    } else {
                        $currentDate = date('d-m-Y');
                    }

                    // Convert the dates to DateTime objects
                    $date1 = \DateTime::createFromFormat('d-m-Y', $checkinDate);
                    ...
                    $monthsDiff = (($year2 - $year1) * 12) + ($month2 - $month1) + 1;
                ...
                $student_rec['totalFeesToPay'] = $totalHostelFee;
                $student_rec['totalFeesPay'] = $totalDepositefee;
                $student_rec['totalPending'] = $student_rec['totalFeesToPay'] - $student_rec['totalFeesPay'] - $student_rec['discount'];
    ...
    // sanjay code for hostel index
    public function hostelcollection()
    {
        $this->viewBuilder()->Layout('admin');
        ...
        foreach ($students as $value) {
            ...
            if ($value['batch'] > '2022-23') {
                ...
                foreach ($asignHostel as $key => $hostelDetails) {
                    ...
                    $totalMonth += $monthsDiff;
                    $totalHostelFee += $monthsDiff * $hostelDetails['amount'];
                    $student_rec['hostel_name'] = $hostelDetails['fee_head_name'];
                    $student_rec['checkinDate'] = $checkinDate;
                }
        ...
        $paginatedData = $this->paginateArray($student_rec_all, 50);
        $this->set('student_rec_all', $paginatedData['data']);
    ...
    // this code for search base hostel fee
    public function hostelcollectionsearch()
    {
        ...
        if (isset($fname) && $fname != '') {
            $cond['Students.st_full_name LIKE'] = '%' . trim($fname) . '%';
        }
        if (isset($fathername) && $fathername != '') {

            $cond['Students.fathername LIKE'] = '%' . trim($fathername) . '%';
        }
        if (isset($class) && $class != '') {
            $cond['Students.class_id'] = $class;
        }
        if (isset($section) && $section != '') {
            $cond['Students.section_id'] = $section;
        }
        if (isset($enroll) && $enroll != '') {
            $cond['Students.enroll'] = $enroll;
        }
        if (isset($batch) && $batch != '') {
            $cond['Students.batch'] = $batch;
        }

        $students = $this->Students->find('all')->contain(['Classes', 'Sections'])->where(['Students.is_hostel IS NOT' => null, 'Students.status' => 'Y', $cond])->order(['Classes.sort' => 'ASC'])->toarray();

        $session = $this->request->session();
        $session->delete('hostelfeesearch');
        $session->write('hostelfeesearch', $cond);
    ...
    public function hostelcollectionhtml()
    {
        $this->viewBuilder()->Layout('newadmin');
        $this->sitesetting('print');
        $this->loadModel('Feesheads');
