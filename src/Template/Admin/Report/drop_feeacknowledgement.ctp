...
$aced=explode('-',$student['updateacedemic']);
...
<tr style="line-height:50px;">
 ...
 <td align="center" colspan="8" style="font-size:14px;" ><b><u>TO WHOM IT MAY CONCERN</u></b></td>
   ...
   $classssr=$this->Comman->findclass($student['laststudclass']); 
  $datse=date('Y-m-d');
  $studentfees=$this->Comman->find_drop_paidamountsack23($student['s_id'],$datse,$student['updateacedemic']);
  //pr($studentfees); die;
 $quas=array();
 $todeposit=0;
 $addtionaldiscount=0;
 $findsum=0;
				...
				foreach($quaf as $j=>$t){
				  ...
				  $findamountmonth=$this->Comman->findamountallmonth($student['class_id'],$student['updateacedemic']);
...
$wsss=$objss->words;								  			   
		$html.=" was studying in Class 
   <b>".$classssr['title']."</b> in 
   our school. The school fee (Tution Fee) for the period April,  ".$aced[0]." 
   to March, ".($aced[0]+1)." is Rs. 
