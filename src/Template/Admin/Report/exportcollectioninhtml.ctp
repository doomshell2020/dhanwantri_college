...
// $sitesetting = $this->Comman->findlogo();
...
// pr($sitesetting['subtitle1']);exit;


// pr($sitesetting['sitetitle']);exit;

// pr( $sitesetting['subtitle2']);exit;
...
<div class="table-responsive">
	<table id="" class="table table-bordered table-striped tableCellWidthMin">
		<tbody>
	        ...
	        <td> <a id="" style=" display: block; width: max-content; margin-left: 0%;" onclick="window.print()"><span class="glyphicon glyphicon-print"></span></a></td>
			</tr>
			<tr>
				<th>S.No</th>
				<th>PayDate</th>
            ...
            if (isset($Classections) && !empty($Classections)) {
				foreach ($Classections as $key => $element) {  //pr($element); die;
									...
									<? if ($element['type'] == "Fee") {
										if ($stiiu['id']) { ?>
											...
											<span>
												...
												<a href="#" <?php if ($stiiu34['status_tc'] == "N") { ?> style="color:orange;" title="L.W.T.C" <?php } else if ($stiiu34['status_tc'] == "Y") { ?> title="W.T.C" style="color:crimson;" <?php } ?>>
													...
													if ($stiius['id']) { ?>
														<span target="_blank" href="<?php echo SITE_URL; ?>admin/studentfees/history/<?php echo $stiius['stud_id']; ?>/<?php echo $element['acedmicyear']; ?>">
