...
<span style="display:block; color:#000; font-size:12px;"><b> <u>Daily Summary Report :- ' . $acedmicyear . '</u></b></span><br>
<span style="display:block; color:#000; font-size:12px;"><b><i><mark style="background-color: yellow;
  color: black;">';
if ($datefrom && $dateto && $datefrom != '1970-01-01' && $dateto != '1970-01-01') {
	$datefromh = $datefrom;
	$dateto2h = $dateto;
...
foreach ($selectField as $j => $el) {
	...
	if ($el == "Discount Fee") {
		...
		if ($el == 'Transport1') {
			$html .= '1st Year Transport Fee';
		} else if ($el == 'Transport2') {
			$html .= '2st Year Transport Fee';
		} else if ($el == 'Transport3') {
			$html .= '3st Year Transport Fee';
		} else if ($el == 'Transport4') {
			$html .= '4st Year Transport Fee';
		} else if ($el == 'Quater1') {
			$html .= '1st Year Tuition Fee';
		} else if ($el == 'Quater2') {
			$html .= '2st Year Tuition Fee';
		} else if ($el == 'Quater3') {
			$html .= '3st Year Tuition Fee';
		} else if ($el == 'Quater4') {
			$html .= '4st Year Tuition Fee';
		} else {
			$html .= ucwords(strtolower($el));
		}
	...
	foreach ($mode as $k => $rt) {
		...
		if ($el == "Registration" && $rt == "CASH") {
			...
			if ($findrecipiet[0]['regfee']) {
				$tot += $findrecipiet[0]['regfee'];
				$aet += $findrecipiet[0]['regfee'];
			...
			if ($findprospectus[0]['p_fees']) {
				...
				$tot += $findprospectus[0]['p_fees'];
				$aet += $findprospectus[0]['p_fees'];
				...
				if ($rt == $value['mode']) {
					...
					foreach ($quas as $iteam['quarter'] => $iteam['amount']) {
						if ($el == "Admission / Prosspectus") {
							if ($iteam['quarter'] == 'Admission / Prosspectus'  && $value['recipetno'] == '0') {
								$fees += '0';
							} else if ($iteam['quarter'] == 'Admission / Prosspectus') {
								$fees += $iteam['amount'];
							}
						} else if ($el == "Collage Caution Money (Refundable)") {
							if ($iteam['quarter'] == 'Collage Caution Money (Refundable)'  && $value['recipetno'] == '0') {
								$cfees += '0';
							} else if ($iteam['quarter'] == 'Collage Caution Money (Refundable)') {
								$cfees += $iteam['amount'];
							}
						} else if ($el == "Uniform") {
							if ($iteam['quarter'] == 'Uniform'  && $value['recipetno'] == '0') {
								$dfees += '0';
...
$amounttyys = money_format('%!i', $tyys);
$html .= $amounttyys;
$html .= '</b></td></tr>  
</tbody></table>';
...
// Check if the remaining height is enough for another table or content
// if ($pdf->GetY() > ($pdf->getPageHeight() - 20)) { // You might want to adjust the margin
...
<table width="100%" border="1" align="center"><tbody>';
	...
	$amousnttyys = money_format('%!i', $ttysk);
	$html .= $amousnttyys;
...
$html .= '<br><br><br><br><br>
<table width="100%" border="1" align="center"><tbody>
<tr><td width="30%"><b>Course Wise Fee Collection</b></td>';
...
foreach ($classes as $key => $value) { 
		...
		$html .= '<tr><td align="left"> ' .$value. ' </td>';
...
ob_end_clean();
$date = date('d-m-Y');
echo $pdf->Output('Daily-Summary-Report-' . $date . '.pdf');
exit;
?>