...
<?php echo $this->Form->create('Defaulter', array('class' => 'form-horizontal', 'enctype' => 'multipart/form-data', 'action' => 'defaultersendsms', 'validate')); ?>
<div class="table-responsive">
  <table id="example1" class="table table-bordered table-striped">
    ...
    text-decoration: overline;"> <? // echo $cho; 
                                  ?></span>
      <tr>
        <td> <input type="button" style="background-color:#00c0ef; position: absolute;top: 74px;margin-left: -8px;" class="btn btn4 btn_pdf myscl-btn date inv globalModals" href="<?php echo SITE_URL; ?>admin/students/sendsms" data-target="#globalModal" data-toggle="modal" name="Invite" value="Send SMS"> </td>
        <td><a id="" style="position: absolute;top: 65px;/* right: 0px; */right: 9px; padding: 7px;" class="btn btn-info btn-sm pull-right" href="<?php echo ADMIN_URL; ?>report/user_defaulter"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Export Excel</a></td>
      </tr>
    ...
    <tbody>
      ...
      $session->delete('resultsss');
      $session->write('resultsss', $results);
      $session = $this->request->session();
      $session->delete('headerRow2');
      $session->write('headerRow2', $headerRow2);
      ...
      ?>
          <tr>
            <td><input type="checkbox" class='checkbox' name="p_id[]" value="<?php echo $rt[1]; ?>" /></td><? for ($i = 0; $i <= $ss; $i++) {  ?>
