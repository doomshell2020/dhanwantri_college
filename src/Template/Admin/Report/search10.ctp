...
foreach($holicount as $key=>$value)
{
 ...
 //   echo $startDate;
   ...
   echo getWorkingDayss($year."-04-01",$year+'1'."-04-01");
 ...
 <br>
  <span style="color:red">Total Holidays Days -<?php echo $tdays; ?>
    ...
    <th>#</th>
    <th>Student-ID</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/4">April</a>(<?php //echo getWorkingDayss($year."-04-01",$year."-04-30"); 
...
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
  if($number_of_days){  echo getWorkingDayss($year."-04-01",$year."-04-30")- $number_of_days;   }          
} 
}else{
	
echo getWorkingDayss($year."-04-01",$year."-04-30");
	} 
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/5">May</a>(<?php //echo getWorkingDayss($year."-05-01",$year."-05-31");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
     if($number_of_days){   echo getWorkingDayss($year."-05-01",$year."-05-31")- $number_of_days;      }  
} 
    }else{
	
echo getWorkingDayss($year."-05-01",$year."-05-31");
	} 
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/6">June</a>(<?php //echo getWorkingDayss($year."-06-01",$year."-06-30");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
  if($number_of_days){ echo getWorkingDayss($year."-06-01",$year."-06-30")- $number_of_days; }             
} 
   }else{
	
echo getWorkingDayss($year."-06-01",$year."-06-30");
	} 
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/7">July</a>(<?php //echo getWorkingDayss($year."-07-01",$year."-07-31");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd); 
      if($number_of_days){ echo getWorkingDayss($year."-07-01",$year."-07-31")- $number_of_days;  }       
}  
}else{
	
	 echo getWorkingDayss($year."-07-01",$year."-07-31");
	}
    
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/8">August</a>(<?php //echo getWorkingDayss($year."-08-01",$year."-08-31");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
       if($number_of_days){    echo getWorkingDayss($year."-08-01",$year."-08-31")- $number_of_days; }        
}
 }else{
	
	 echo getWorkingDayss($year."-08-01",$year."-08-31");
	}
    
    
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/9">September</a>(<?php //echo getWorkingDayss($year."-09-01",$year."-09-30");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
          if($number_of_days){   echo getWorkingDayss($year."-09-01",$year."-09-30")- $number_of_days; }        
} }else{
	
	 echo getWorkingDayss($year."-09-01",$year."-09-30");
	} 
    
    
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/10">October</a>(<?php 
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
          if($number_of_days){ echo getWorkingDayss($year."-10-01",$year."-10-31")- $number_of_days;  }       
} 
}else{
	
	 echo getWorkingDayss($year."-10-01",$year."-10-31");
	} 
    
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/11">November</a>(<?php //echo getWorkingDayss($year."-11-01",$year."-11-30");  
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
        if($number_of_days){  echo getWorkingDayss($year."-11-01",$year."-11-30")- $number_of_days; }        
} 
}else{
	
	 echo getWorkingDayss($year."-11-01",$year."-11-30");
	} 
    
    
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/12">December</a>(<?php //echo getWorkingDayss($year."-12-01",$year."-12-31");  
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
           if($number_of_days){   echo getWorkingDayss($year."-12-01",$year."-12-31")- $number_of_days; }   
} 
}else{
	
	 echo getWorkingDayss($year."-12-01",$year."-12-31");
	} 
    
    ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/1">January</a>(<?php //echo getWorkingDayss($year+'1'."-01-01",$year+'1'."-01-31");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
   if($number_of_days){    echo getWorkingDayss($year+'1'."-01-01",$year+'1'."-01-31")- $number_of_days; }         
} 
  }else{
	
	 echo getWorkingDayss($year+'1'."-01-01",$year+'1'."-01-31");
	}   
    ...
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/2">February</a>(<?php if( (0 == $year+1 % 4) and (0 != $year+1 % 100) or (0 == $year+1 % 400) )
		{ //echo getWorkingDayss($year+'1'."-02-01",$year+'1'."-02-29");   
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
           		   if($number_of_days){  echo getWorkingDayss($year+'1'."-02-01",$year+'1'."-02-29")- $number_of_days;   }
		} 
	 ...
	 echo getWorkingDayss($year+'1'."-02-01",$year+'1'."-02-29");
		 ...
		 }else {  //echo getWorkingDayss($year+'1'."-02-01",$year+'1'."-02-28");   
		      if(!empty($mholicount)){
		 $mholicount=$this->Comman->findmonthholicount('8','2',$year+1);  $number_of_days=0;
foreach($mholicount as $keyed => $value)
{
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
           if($number_of_days){   echo getWorkingDayss($year+'1'."-02-01",$year+'1'."-02-28")- $number_of_days;   }
		
		  }  
		  }else{
	
echo getWorkingDayss($year+'1'."-02-01",$year+'1'."-02-28")- $number_of_days;
	}   
		  
		   } ?>)</th>
    <th><a href="<?php echo SITE_URL; ?>admin/report/register/<?php echo $mid; ?>/3">March</a>(<?php //$w=getWorkingDayss($year+'1'."-03-01",$year+'1'."-03-31");
...
$end_dated=date('Y-m-d',strtotime($value['endtime']));
$number_of_days = calculateHolidays($start_dated, $end_dated, $monthd);
            if($number_of_days){   echo getWorkingDayss($year+'1'."-03-01",$year+'1'."-03-31")- $number_of_days;   }
} 
  }else{
	
 echo getWorkingDayss($year+'1'."-03-01",$year+'1'."-03-31");
	} 
    
    ?>)</th>
    <th>Total(<?php echo getWorkingDayss($year."-04-01",$year+'1'."-04-01")-$tdays;?>)</th>
    </tr>
<?php
//get current month for exampl
function getWorkingDayss($startDate, $endDate)
{
