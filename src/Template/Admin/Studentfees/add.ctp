<!-- Content Wrapper. Contains page content -->
        ...
        <div class="col-md-12">
          ...
          <div class="box box-info">
		    <div class="box-header with-border">
		      <h3 class="box-title"><?php  if(isset($classes['id'])){ echo 'Edit Class Fee'; }else{ echo 'Add Class Fee';} ?></h3>
		    </div>
            <!-- /.box-header -->
            <!-- form start -->
 ...
 <label>Select Class</label>
   	<?php if($seletedclassid){  
		echo $this->Form->input('class_id',array('class'=>'form-control','type'=>'select','empty'=>'Select Class','options'=>$classlist,'value'=>$seletedclassid,'label' =>false,'required')); }else{  
 ...
 <?= $year=date("Y");  $exyear=$year+4; ?>
  ...
  <?php for ($i = $year; $i <= $exyear; $i++) :  $rt=$i+1; $rt= substr($rt,2);?> 
    ...
    <?php endfor; ?>
		...
		foreach($feesheads as $key=>$work){ ?>
			<tr>
                            <td class="text-center text-bold">
								<input type="hidden" name="fee_h_id[]" value="<?php echo $work['id']; ?>"><?php echo $work['name']; ?></td>
                            <td class="text-center text-bold"><input type="text" name="qu1_fees[]" ></td>
                            <td class="text-center text-bold"><input type="text" name="qu2_fees[]" ></td>
                            <td class="text-center text-bold"><input type="text" name="qu3_fees[]" ></td>
                            <td class="text-center text-bold"><input type="text" name="qu4_fees[]" ></td>
