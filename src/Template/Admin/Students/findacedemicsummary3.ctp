...
<section class="content-header">
   <h1>
      <i class="fa fa-money"></i> <?php echo $acedmic; ?> <?php $rolepresent=$this->request->session()->read('Auth.User.role_id');
         if($rolepresent==CBSE_FEE_COORDINATOR){   ?> Admission Detail Migrated From International Summary <?php }else{ ?> Admission Detail Migrated From CBSE Summary <?php } ?> 
   </h1>
   <ol class="breadcrumb">
...
<table id="" class="table table-bordered table-striped">
   <tbody>
      ...
      <tr>
         ...
         <td><?php if($element['f_phone']!=''){ echo $element['f_phone'];  }else{  echo "0"; } ?></td>
         <td><?php if($element['m_phone']!=''){  echo $element['m_phone']; }else{  echo "0"; }  ?></td>
      </tr>
      <?php  }
         }
