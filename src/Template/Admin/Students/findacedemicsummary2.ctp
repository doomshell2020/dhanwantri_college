...
<section class="content-header">
   <h1>
      <i class="fa fa-money"></i> <?php echo $acedmic; ?> <?php $rolepresent = $this->request->session()->read('Auth.User.role_id');
         if ($rolepresent == CBSE_FEE_COORDINATOR) {   ?> Admission Detail Migrated To International Summary <?php } else { ?> Admission Detail Migrated To CBSE Summary <?php } ?>
   </h1>
   <ol class="breadcrumb">
...
<table id="" class="table table-bordered table-striped">
   <tbody>
      ...
      <tr>
         ...
         <td><?php if ($element['created'] != '') {
            echo date('d-m-Y', strtotime($element['created']));
            } else {
            echo "N/A";
            }  ?></td>
         ...
         <td><?php echo $element['fname'] . ' ' . $element['middlename'] . ' ' . $element['lname'];
            if ($element['oldenroll'] != 0 || $element['category'] == "Migration") {
            	echo "<span style='color: red;'>(Migr.)*</span>";
            } ?></td>
         <td><?php echo $element['fee_submittedby'];  ?></td>
         <td><?php $classs = $this->Comman->findclass($element['class_id']);
            echo $classs['title'];  ?></td>
         <td><?php if ($element['board_id'] == '1') {
            echo "CBSE";
            } else {
            echo "INTERNATIONAL";
            } ?></td>
         <td><?php if ($element['sms_mobile'] != '') {
            echo $element['sms_mobile'];
            } else {
            echo "N/A";
            } ?></td>
         <?php } ?>
