...
<? if ($this->request->session()->read('Auth.User.role_id') == '1') {
   if (isset($classess) && !empty($classess)) {
   	...
   	foreach ($classess as $element) {
   		...
   		$cbntsf = 0;
   		foreach ($findstudenttotlaclasss as $hs => $js) {
   			$cbntsf++;
   		}
...
<table id="" class="table table-bordered table-striped">
   <tbody>
      ...
      <tr>
         ...
         <td> <?
            $findstudenttotlacslasssabsent = $this->Comman->absentclasstodayreportss23date($element['class_id'], $element['section_id'], $date);
            $classstodayreportss23 = $this->Comman->classtodayreportss23date($element['class_id'], $element['section_id'], $date);
            ...
            ?><a target="_blank" href="<?php echo SITE_URL; ?>admin/students/searchabsent/<?php echo $element['class_id']; ?>/<?php echo $element['section_id']; ?>/<?php echo $date; ?>"> <? $total_absent += $findstudenttotlacslasssabsent;
            echo $findstudenttotlacslasssabsent; ?></a> <? } else {
