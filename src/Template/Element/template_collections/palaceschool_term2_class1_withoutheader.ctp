...
<table width="100%">
<tr>
<td align="left" width="22%"  style="height:20px; line-height:20px;border-top:1px solid #000; border-left:1px solid #000;  border-right:1px solid #000;"><h2 style=" font-size:10px; font-weight:bold; ">&nbsp; Scholastic Areas:</h2></td>
<td align="center" width="78%" style=" border-top:1px solid #000; height:20px; line-height:20px;  font-weight:bold; border-right:1px solid #000;">Annual Assessment (100 Marks)</td>
</tr>
</table>
<table width="100%" >';
    $findroleexamtype1 = $this->Comman->findexamtypesclass($students['class_id'], 2);
    // pr($findroleexamtype1);die;
    $html .= '<tr style="font-size:11px;">
<td width="22%" style="border-top:1px solid #000; border-left:1px solid #000;  height:32px; line-height:32px;  border-right:1px solid #000; text-align:left;  font-weight:bold; " align="left">&nbsp; Subject</td>';

    $cbg = 0;
    foreach ($findroleexamtype1 as $key => $item) {
        $html .= '<td width="13%" style="border-top:1px solid #000; height:16px; line-height:14px;  font-weight:bold;  border-right:1px solid #000;  text-align:center;" align="center">' . $item['examprint'] . '<br>(' . $item['maxnumber'] . ')</td>';
        $cbg += $item['maxnumber'];
    }

    $html .= '<td width="13%" style="border-top:1px solid #000; height:16px; line-height:14px;  font-weight:bold;  border-right:1px solid #000;  text-align:center;" align="center">Marks Obtained<br>(100)</td>

<td width="13%" style="border-top:1px solid #000; height:16px; line-height:14px;  font-weight:bold; border-right:1px solid #000; text-align:center;" align="center">Grade</td>';
    ...
    foreach ($subject as $key => $name) {
        ...
        $html .= '</tr><tr style="font-size:11px;"><td width="22%" style="border-top:1px solid #000; border-left:1px solid #000;  height:20px; line-height:20px; border-bottom:1px solid #000;  border-right:1px solid #000; text-align:center;  font-weight:bold; " align="left">&nbsp; ' . $name['exprint'] . '</td>';
        ...
        foreach ($findroleexamtype as $er) {
            ...
            if ($newmarks['marks'] != '') {
                ...
                $html .= '
                <td width="13%" style="border-top:1px solid #000;   height:20px; line-height:20px; border-bottom:1px solid #000;  border-right:1px solid #000;  text-align:center;" align="center">' . $newmarks['marks'] . '</td>
                ';
                $ter += $newmarks['marks'];
                // $addall += $ter;
                // pr($addall);
            } else {
                $html .= '<td width="13%" style="border-top:1px solid #000;   height:20px; line-height:20px;  border-right:1px solid #000;  text-align:center;" align="center">--</td>';
            }
        }
        $html .= '<td width="13%"  style="border-top:1px solid #000;  border-right:1px solid #000; border-bottom:1px solid #000; height:20px; line-height:20px;" align="center">' . round($ter) . '</td>';
      
        $getgrade = gradecalculation($ter, 100, 100.0);

        $html .= '<td width="13%"  style="border-top:1px solid #000;  border-right:1px solid #000; border-bottom:1px solid #000; height:20px; line-height:20px;" align="center">' . $getgrade . '</td>';
    }
    $html .= '</tr>';
