...
function gradecalculation($marks, $maxnumber, $number)
{
    ...
    if ($marks != 0) {
        ...
        $grades = round($marks / $maxnumber * $number);
        // pr($grades);
        // die;
...
<table width="100%" ><tr>
<td width="51%" style="text-transform:uppercase; font-size:14px; font-weight:bold; text-transform:uppercase; text-align:center; border:1px solid #000">
Term 1

</td>

<td width="49%"style="text-transform:uppercase; font-size:14px; font-weight:bold; text-transform:uppercase; text-align:center; border:1px solid #000">
Term 2
</td>
</tr>';
    ...
    $html .= '<td width="8%" style="border-top:1px solid #000; height:16px; line-height:16px;  font-weight:bold;  border-right:1px solid #000;  text-align:center;" align="center">Total Marks<br>(' . $cbg . ')</td>';
    ...
    $html .= '<td width="8%" style="border-top:1px solid #000; height:16px; line-height:16px;  font-weight:bold; border-right:1px solid #000; text-align:center;">Total Marks<br>(' . $cbg2 . ')</td>
...
<td width="8%" style="border-top:1px solid #000; height:32px; line-height:32px;  font-weight:bold; border-right:1px solid #000; text-align:center;">Grade</td>
</tr>';
    ...
    foreach ($subject as $key => $name) {
        ...
        <td style="border-top:1px solid #000;border-bottom:1px solid #000; border-left:1px solid 
        #000; height:15px; line-height:15px; border-right:1px solid #000; " 
        align="center">&nbsp; ' . $name['exprint'] . '</td>';


        $findroleexamtype = $this->Comman->findeexamtsype3s($students['sect_id'], $students['class_id'], 1, $students['student']['acedmicyear']);
        // pr($findroleexamtype);exit;
        $ter = 0;
        // $term1 = 0;
        foreach ($findroleexamtype as $er) {

            $newmarks = $this->Comman->fsubjectmarks2($students['stud_id'], $students['sect_id'], $students['class_id'], $er['etype_id'], $name['id'], 1, $students['student']['acedmicyear']);
            if ($newmarks['marks'] != '') {
        ...
        $findroleexamtype2 = $this->Comman->findeexamtsype3s($students['sect_id'], $students['class_id'], 2, $students['student']['acedmicyear']);
        $ter2 = 0;
        $ter3 = 0;
        foreach ($findroleexamtype2 as $er) {

            $newmarks = $this->Comman->fsubjectmarks2($students['stud_id'], $students['sect_id'], $students['class_id'], $er['etype_id'], $name['id'], 2, $students['student']['acedmicyear']);
...
#000; height:18px; border-bottom:1px solid #000; line-height:18px; font-size:11px; font-weight:bold; text-align:center;">TOTAL</td>';
    ...
    $totalgrand2 = 0;
    $iiExam = 1;
    foreach ($findroleexamtype1 as $newkey => $item) {
        // pr($item); die;
        $theorymarks1 = 0;
        $practicalmarks = 0;
        $gtotal = 0;
        foreach ($subject as $key => $name) {
            ...
            $newmarks = $this->Comman->fsubjectmarks2($students['stud_id'], $students['sect_id'], $students['class_id'], $item['id'], $name['id'], 1, $students['student']['acedmicyear']);
            if ($newmarks['marks'] != '') {
                $gtotal += $newmarks['marks'];
                $theorymarks1 += $newmarks['exam_subject']['theorymarks'];
                $practicalmarks += $newmarks['exam_subject']['practicalmarks'];
            }
        }
        // pr($theorymarks1);
        // pr($practicalmarks);die;
        $grades = 0;
        $sss = '';
        if ($newkey == 0) {
            $getgrade = gradecalculation($gtotal, $theorymarks1, 100.0);
        } else if ($newkey == 1) {
            $getgrade = gradecalculation($gtotal, $practicalmarks, 100.0);
        } else {
            $getgrade = gradecalculation($gtotal, 250, 100.0);
        }
        $sss = $getgrade;
    ...
    if ($totalgrand1 != 0) {
        $subjectotal1 = 0;
        $subjectotal1 = $subjectcnt * 150;
        // echo $totalgrand1;die;
        $grades1 = $totalgrand1 * 100 / $subjectotal1;
        $ssss = gradecalculation($totalgrand1, $subjectotal1, 100.0);
    }
    ...
    foreach ($findroleexamtype2s as $k => $item) {
        ...
        foreach ($subject as $key => $name) {
            ...
            $newmarks = $this->Comman->fsubjectmarks2($students['stud_id'], $students['sect_id'], $students['class_id'], $item['id'], $name['id'], 2, $students['student']['acedmicyear']);
            // pr($newmarks);
            if ($newmarks['marks'] != '') {
                $gtotals += $newmarks['marks'];
                $theorymarks2 += $newmarks['exam_subject']['theorymarks'];
                $practicalmarks2 += $newmarks['exam_subject']['practicalmarks'];
            }
        }
        // if($k==2){
        //     $getgrade = gradecalculation($gtotals, $practicalmarks2, 100.0);
        //     $sss = $getgrade;
        //     pr($getgrade);exit;
        // }

        if ($k == 0) {
            $getgrade = gradecalculation($gtotals, $theorymarks2, 100.0);
            $sss = $getgrade;
            // pr($getgrade);exit;
        } else if ($k == 1) {
            $getgrade = gradecalculation($gtotals, $practicalmarks2, 100.0);
            ...
            $getgrade = gradecalculation($gtotals, 250, 100.0);
            $sss = $getgrade;
        }
        if ($gtotals != 0) {
    ...
    if ($totalgrand2 != 0) {
        ...
        // pr($totalgrand2);exit;
        $subjectotal2 = 0;
        $subjectotal2 = $subjectcnt * 150;
        // $grades2 = $totalgrand2 * 100 / $subjectotal2;
        $ssss2 = gradecalculation($totalgrand2, $subjectotal2, 100.0);
    }
...
#000; height:18px; border-bottom:1px solid #000; line-height:18px; font-size:11px; font-weight:bold; text-align:center;">' . $totalgrand2 . ' ' . $ssss2 . ' </td>';
    ...
    if ($grand != 0) {
        $subjectotal3 = 0;
        $subjectotal3 = $subjectcnt * 300;
        //echo $subjectotal;
        $grades3 = $grand * 100 / $subjectotal3;
        $ssss3 = gradecalculation($grand, $subjectotal3, 100.0);
    }
...
#000; height:18px; border-bottom:1px solid #000; line-height:18px; font-size:11px; font-weight:bold; text-align:center;">' . $grand . ' ' . $ssss3 . ' </td>';
    ...
    $html .= '<td rowspan="2" style="border-top:1px solid #000; border-right:1px solid 
...
#000; border-left:1px solid #000; height:18px; border-bottom:1px solid #000; line-height:18px; font-size:11px; font-weight:bold; text-align:center;">TOTAL %</td>';
    ...
    $totalgrand2 = 0;
    $iiExam = 1;
    foreach ($findroleexamtype1 as $r => $item) {
        $gtotal = 0;
        $theorymarks3 = 0;
        $practicalmarks3 = 0;
        foreach ($subject as $key => $name) {
            ...
            if ($newmarks['marks'] != '') {
                $gtotal += $newmarks['marks'];
                $theorymarks3 += $newmarks['exam_subject']['theorymarks'];
                $practicalmarks3 += $newmarks['exam_subject']['practicalmarks'];
            }
        }
        ...
        $sss = '';
        if ($r == 0) {
            $marksnew1 = $theorymarks3;
            $grades = $gtotal * 100 / $marksnew1;
        } else if ($r == 1) {
            $marksnew2 = $practicalmarks3;
            $grades = $gtotal * 100 / $marksnew2;
        } else {
            $marksnew3 = 250;
            $grades = $gtotal * 100 / $marksnew3;
        }
        // pr($gtotal);
        if ($gtotal != 0) {
            $totalgrand1 += number_format($gtotal);
            ...
            if ($iiExam == 1) {
                ...
                // $grades = $gtotal * 100 / $theorymarks;
                // pr($grades);exit;
                // $sss = gradecalculation($grand, $subjectotal3, 100.0);
            }
        }
...
#000; height:18px; border-bottom:1px solid #000; line-height:18px; font-size:11px; font-weight:bold; text-align:center;">' . number_format($grades1, 2) . ' </td>';
    //total percentage calculate term 1 end
    ...
    foreach ($findroleexamtype2s as $k => $item) {
        ...
        foreach ($subject as $key => $name) {
            ...
            if ($newmarks['marks'] != '') {
                $gtotals += $newmarks['marks'];
                $theorymarks4 += $newmarks['exam_subject']['theorymarks'];
                $practicalmarks4 += $newmarks['exam_subject']['practicalmarks'];
            }
        }

        if ($k == 0) {
            $marksnew1 = $theorymarks4;
            // pr();exit;
            $grades = $gtotals * 100 / $marksnew1;
        } else if ($k == 1) {
            $marksnew2 = $practicalmarks4;
            $grades = $gtotals * 100 / $marksnew2;
        } else {
            $marksnew3 = 250;
            $grades = $gtotals * 100 / $marksnew3;
        }
...
#000; height:18px; border-bottom:1px solid #000; line-height:18px; font-size:11px; font-weight:bold; text-align:center;">' . number_format($grades3, 2) . ' </td>';
    ...
    // student Attendence 
    $attendncterm1 = $this->Comman->findstuattendetails($students['student']['id'], $students['class_id'], $students['sect_id'], $students['student']['acedmicyear'], 1, $students['student']['acedmicyear']);
    $attendncterm2 = $this->Comman->findstuattendetails($students['student']['id'], $students['class_id'], $students['sect_id'], $students['student']['acedmicyear'], 2, $students['student']['acedmicyear']);

    // $WorkingDays = $attendncterm1['total'] + $attendncterm2['total'];
    $WorkingDays = $attendncterm2['total'];
    // pr($WorkingDays);exit;
    // $daysAttended = $attendncterm1['present'] + $attendncterm2['present'];
    $daysAttended = $attendncterm2['present'];
