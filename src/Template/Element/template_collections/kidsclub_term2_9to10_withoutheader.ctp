...
function calculateGrade($obtained, $totalmarks)
{
	$ter3 = $obtained;
	$grade = 0;
	$ss = "";
...
foreach ($studentsj as $f => $students) {
	...
	// $ngo_students_enrollment = ['1799', '1800', '1802', '1803', '1804', '1805', '1806', '1807', '1808', '1809', '1810', '1811'];
	// pr($ngo_students_enrollment);die;
	...
	$html .= '<tr style="font-size:11px;"><td width="22%" style="border-top:1px solid #000; border-left:1px solid #000;  height:32px; line-height:32px;  border-right:1px solid #000; text-align:center;  font-weight:bold; " align="center">&nbsp; Subject</td>';
	...
	foreach ($findroleexamtype2 as $key => $item) {
		...
		$html .= '<td width="12%" style="border-top:1px solid #000; height:16px; line-height:16px;  font-weight:bold;  border-right:1px solid #000;  text-align:center;" align="center">' . $item['examprint'] . '<br>(' . $item['maxnumber'] . ')</td>';
		$cbg2 += $item['maxnumber'];
	}
	$total = $cbg2;

	$html .= '
	<td width="15%" style="border-top:1px solid #000; height:16px; line-height:16px;  font-weight:bold; border-right:1px solid #000; text-align:center;">Grand<br>Total<br>(' . $total . ')</td>

	<td width="15%" style="border-top:1px solid #000; height:32px; line-height:32px;  font-weight:bold; border-right:1px solid #000; text-align:center;">Grade</td>
	</tr>';
	...
	foreach ($subject as $key => $name) {
		...
		<td width="22%" style="border-top:1px solid #000;border-bottom:1px solid #000; border-left:1px solid 
		...
		foreach ($findroleexamtype2 as $er) {
			...
			$newmarks = $this->Comman->fsubjectmarks2($students['stud_id'], $students['sect_id'], $students['class_id'], $er['id'], $name['id'], 2, $students['student']['acedmicyear']);

			if ($newmarks['marks'] != '') {

				$html .= '<td width="12%"  style="border-top:1px solid #000;  border-right:1px solid 
		...
		$ss = "--";
		if ($ter3 != 0) {
			$ss = calculateGrade($ter3, 100);
		}

		$html .= '<td width="15%"  style="border-top:1px solid #000;  border-right:1px solid 
					#000; border-bottom:1px solid #000; height:15px; line-height:15px;" 
					align="center">' . $ter3 . '</td>';

		$marksobtastu += $ter3;
		$html .= '<td width="15%"  style="border-top:1px solid #000;  border-right:1px solid 
				#000; border-bottom:1px solid #000; height:15px; line-height:15px;" align="center">' . $ss . '</td>';
	...
	$overallgrades = "--";
	$overallgrades = calculateGrade($marksobta, $totalmarkss);
...
// echo $html; die;
ob_end_clean();
echo $pdf->Output(ucwords(strtolower($students['student']['fname'])) . '_' . ucwords(strtolower($students['student']['middlename'])) . '_' . ucwords(strtolower($students['student']['lname'])) . '_' . $students['student']['acedmicyear'] . '.pdf');
exit;
